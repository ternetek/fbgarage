...
- **`sections`** — cada sección tiene un `id` (para anchors), `title` y según tipo:
  ...
  - **`items`** — objetos `{ "name", "price", "description" }`. Opcional **`image`**: `"menu/dishes/archivo.jpeg"` o `{ "src", "alt" }`. Opcional **`imageFocus`**: `"top"`, `"tc"` (entre top y center), `"bottom"` o `"bc"` (entre bottom y center) para anclar el recorte de `object-fit: cover`. Sin valor = center.
